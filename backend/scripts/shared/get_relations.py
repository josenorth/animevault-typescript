...
def insert_relations(data):
    try:
        # Verificar e insertar medias principales
        for media in data['data']['Page']['media']:
            media_id = media['id']
            media_type = media['type']

            # Insertar media principal si no existe
            if not exists_in_database(session, media_type, media_id):
                if media_type == 'ANIME':
                    anime_data = fetch_anime_by_id(media_id)
                    if anime_data:
                        insert_anime(session, anime_data)
                        session.commit()
                        logger.info(f"Inserted new {media_type}: {media_id}")
                else:  # MANGA
            ...
            # Verificar e insertar medias relacionados
            if 'relations' in media and media['relations']['edges']:
                for edge in media['relations']['edges']:
                    related_id = edge['node']['id']
                    related_type = edge['node']['type']

                    if not exists_in_database(session, related_type, related_id):
                        if related_type == 'ANIME':
                            related_data = fetch_anime_by_id(related_id)
                            if related_data:
                                insert_anime(session, related_data)
                                session.commit()
                                logger.info(f"Inserted new related {related_type}: {related_id}")
                        else:  # MANGA
                            related_data = fetch_manga_by_id(related_id)
                            if related_data:
                                insert_manga(session, related_data)
                                session.commit()
                                logger.info(f"Inserted new related {related_type}: {related_id}")
        ...
        for media in data['data']['Page']['media']:
            ...
            for edge in media['relations']['edges']:
                ...
                if not relation_exists(session, **check_params):
                    ...
                    if id_exists(session, check_params):
